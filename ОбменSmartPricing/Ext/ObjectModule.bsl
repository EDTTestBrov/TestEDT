...
Процедура ВыполнитьОбменSmartPricing()
	...
	Если ЭтотОбъект.РежимЗагрузки Тогда
		...
		Попытка
			Соединение = Новый FTPСоединение(АдресСервераFTP, ?(ЗначениеЗаполнено(ПортСервераFTP),ПортСервераFTP,20), Пользователь, Пароль);
...
Функция ПолучитьДанныеПоАссортименту(НачалоПериода,КонецПериода,Разделитель)
	...
	запрос = новый запрос("ВЫБРАТЬ
	                      ...
	                      |	ВТСтатусыНоменклатуры.Номенклатура.Родитель.Родитель.Родитель.Родитель КАК ГруппаУровень21,
	                      |	ВТСтатусыНоменклатуры.Номенклатура.Родитель.Родитель.Родитель.Родитель.Код КАК КодГруппаУровень22,
	                      |	ВТСтатусыНоменклатуры.Номенклатура.Родитель.Родитель.Родитель.Родитель.ИДВнешнейСистемы КАК ИдГруппаУровень32,
